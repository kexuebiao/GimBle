...
#ifndef GIMKIT_SWIFT_H
#define GIMKIT_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="GimKit",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC6GimKit9AudioItem")
...
SWIFT_CLASS("_TtC6GimKit11AudioPlayer")
...
SWIFT_CLASS("_TtC6GimKit13GimKitManager")
@interface GimKitManager : NSObject
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
@interface GimKitManager (SWIFT_EXTENSION(GimKit)) <CBCentralManagerDelegate>
/// GimKit uses this to figure out whether Bluetooth is available or not.
/// <ul>
///   <li>
...
/// When connected, update GimKit’s states by updating the values for <code>connectingPeripheral</code>.  Also, make sure to broadcast the event to observers, and notify the queue so that the current operation in-flight can process this event and get a chance to finish.
- (void)centralManager:(CBCentralManager * _Nonnull)central didConnectPeripheral:(CBPeripheral * _Nonnull)peripheral;
/// Handle a disconnection event from Core Bluetooth by figuring out what kind of disconnection it is (planned or unplanned), and updating GimKit’s internal state and sending notifications as appropriate.
- (void)centralManager:(CBCentralManager * _Nonnull)central didDisconnectPeripheral:(CBPeripheral * _Nonnull)peripheral error:(NSError * _Nullable)error;
/// This mostly happens when either the Bluetooth device or the Core Bluetooth stack somehow only partially completes the negotiation of a connection. For simplicity, GimKit is currently treating this as a disconnection event, so it can perform all the same clean up logic.
